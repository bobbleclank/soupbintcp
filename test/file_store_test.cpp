...
TEST(File_store, add_to_existing_file) {
  unlink(filename);
  {
    ...
    ec = s.add("brown", 5);
    ASSERT_FALSE(ec);
    ASSERT_EQ(s.next_sequence_number(), 4);
  ...
  }
  {
    ...
    ec = s.add("over", 4);
    ASSERT_FALSE(ec);
    ASSERT_EQ(s.next_sequence_number(), 7);
...
TEST(File_store, move_operations) {
  ...
  auto ec = s1.open();
  ASSERT_FALSE(ec);
  ASSERT_EQ(s1.next_sequence_number(), 1);
  ...
  ec = s1.add("brown", 5);
  ASSERT_FALSE(ec);
  ASSERT_EQ(s1.next_sequence_number(), 4);

  File_store s2(std::move(s1));
  ASSERT_EQ(s1.next_sequence_number(), 1);
  ASSERT_EQ(s2.next_sequence_number(), 4);
  ...
  ec = s2.add("over", 4);
  ASSERT_FALSE(ec);
  ASSERT_EQ(s2.next_sequence_number(), 7);

  File_store s3;
  s3 = std::move(s2);
  ASSERT_EQ(s2.next_sequence_number(), 1);
  ASSERT_EQ(s3.next_sequence_number(), 7);

  File_store s4(filename_2);
  ec = s4.open();
  ASSERT_FALSE(ec);
  ASSERT_EQ(s4.next_sequence_number(), 1);
  ...
  ec = s4.add("quickly", 7);
  ASSERT_FALSE(ec);
  ASSERT_EQ(s4.next_sequence_number(), 7);

  s4 = std::move(s3);
  ASSERT_EQ(s3.next_sequence_number(), 1);
  ASSERT_EQ(s4.next_sequence_number(), 7);
  ...
  ec = s4.add("dog", 3);
  ASSERT_FALSE(ec);
  ASSERT_EQ(s4.next_sequence_number(), 10);
