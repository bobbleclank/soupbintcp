...
struct Partial_read : Partial_rw<unsigned char> {
  auto read() {
    return detail::read_partial_handling(fd, arr.data(), arr.size(), *this);
  }
};

struct Partial_write : Partial_rw<const unsigned char> {
  auto write() {
    return detail::write_partial_handling(fd, arr.data(), arr.size(), *this);
  }
};
...
TEST(File_store, read_partial_handling) {
  {
    ...
    Partial_read p{{in}};
    auto res = p.read();
    ASSERT_EQ(res.status, detail::Read_status::success);
  ...
  }
  {
    ...
    Partial_read p{{in}};
    auto res = p.read();
    ASSERT_EQ(res.status, detail::Read_status::failure);
  ...
  }
  {
    ...
    Partial_read p{{in}};
    auto res = p.read();
    ASSERT_EQ(res.status, detail::Read_status::end_of_file);
    ASSERT_EQ(res.nbyte, 0u);
    ASSERT_EQ(p.out.size(), 1u);
    auto i = p.out.begin();
    ASSERT_EQ(i->pos, 0);
...
TEST(File_store, write_partial_handling) {
  {
    ...
    Partial_write p{{in}};
    auto res = p.write();
    ASSERT_EQ(res.status, detail::Write_status::success);
    ASSERT_EQ(res.nbyte, 10u);
    ASSERT_EQ(p.out.size(), 1u);
    auto i = p.out.begin();
    ASSERT_EQ(i->pos, 0);
  ...
  }
  {
    ...
    Partial_write p{{in}};
    auto res = p.write();
    ASSERT_EQ(res.status, detail::Write_status::failure);
    ASSERT_EQ(res.nbyte, 0u);
    ASSERT_EQ(p.out.size(), 1u);
    auto i = p.out.begin();
    ASSERT_EQ(i->pos, 0);
