...
TEST(packing, pack_password) {
  ...
  constexpr auto size = pad + 10 + pad;
  auto pack = [](auto s, auto b) { pack_password(s, b); };
  {
    std::string_view s = "";
...
TEST(packing, unpack_password) {
  ...
  auto unpack = [](auto& s, auto b) { unpack_password(s, b); };
  {
    std::string_view b = "          ";
...
TEST(packing, pack_session) {
  constexpr auto pad = 3;
  constexpr auto size = pad + 10 + pad;
  auto pack = [](auto s, auto b) { pack_session(s, b); };
  {
    std::string_view s = "";
    std::array<char, size> b = {};
    b.fill('.');
...
TEST(packing, unpack_session) {
  constexpr auto size = 10;
  constexpr auto extra = 2;
  auto unpack = [](auto& s, auto b) { unpack_session(s, b); };
  {
    std::string_view b = "          ";
    ASSERT_EQ(b.size(), size);
    std::string s;
...
TEST(packing, pack_sequence_number) {
  constexpr auto pad = 3;
  constexpr auto size = pad + 20 + pad;
  auto pack = [](auto i, auto b) { pack_sequence_number(i, b); };
  {
    std::uint64_t i = 0;
    std::array<char, size> b = {};
    b.fill('.');
...
TEST(packing, unpack_sequence_number) {
  ...
  auto unpack = [](auto& i, auto b) { unpack_sequence_number(i, b); };
  {
    std::string_view b = "                   0";
    ASSERT_EQ(b.size(), size);
