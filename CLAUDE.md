...
- `Connection_state` — pure state machine (no side effects) tracking `State` and `reason_`. Used by both client and server `Tcp_connection`. Default-constructs to `State::connecting`; the server uses the parameterized constructor (`State::connected`) since it's born post-accept. Functions: `set_state` (free transitions), `initiate_disconnect` (state → disconnecting), `disconnect` (state → disconnected). The disconnect variants return whether the state changed; callers handle socket close and callbacks. **The first reason set wins** — `initiate_disconnect` stamps `reason_` unconditionally, while `disconnect` only assigns when it is still `none`. So a reason stamped up front survives whatever the connection observes later.
- `Socket` — wraps Asio TCP socket, calls back into a `Socket::Handler`. Tracks pending async ops (`connect_pending_`, `read_pending_`, `write_pending_`) and signals `closed()` on the Handler once all ops have drained after `close()`. Async ops are no-ops when `closing_` is set. Outgoing packets queue in `write_packets_` (a `std::list`), capped at `write_packets_limit_` — user-controlled via `Acceptor`/`Client`'s `set_write_packets_limit` (default 100). Two write entry points: `async_write` (returns `buffer_full` at `size >= write_packets_limit_`) and `async_write_guaranteed` (bypasses the cap, always queues, still a no-op when `closing_`). In `async_write`, the `>=` rather than `==` is required, not defensive — `async_write_guaranteed` can push the queue past the cap. See [Login-flow delivery guarantee](#login-flow-delivery-guarantee).
...
A static, library-owned debug greeting, set per config-level object and sent automatically as the connection opens. Three levels: `Acceptor` (per-endpoint, server), `Port` (per-user, server), `Connection` (per-host, client).

- **Ahead of the login packet — by design.** The protocol defines the login request/response (`login_request`/`login_accepted`/`login_rejected`) as the "first *non-debug* packet"; debug may precede it.
- **Best-effort** — `(void)`-discarded; losing a diagnostic greeting is harmless. The must-deliver login packet is protected separately (see [Login-flow delivery guarantee](#login-flow-delivery-guarantee)).
- **Connection-level on the client** (not Client-level): the redundant physical connections go to different hosts, so the banner is per-host config, like the connection's credentials.
- **Server pre-login access asymmetry:** the server user has no `Port` until login, so the `connected`-state banners (acceptor and port) are the *only* pre-login debug there — the user can't reach the connection to send any. The client user *does* hold the `Connection` pre-login, so they can send debug themselves.

## Login-flow delivery guarantee
...
**Server pre-login routing.** A pre-login-capable callback checks `handler_` (the `Port_handler`): if set (logged in), the per-port handler fires; otherwise the per-acceptor handler (`acceptor_handler_`) fires — or drops, when there's no acceptor-level equivalent. So `debug` / `transport_error` / `protocol_violation` → `Acceptor_handler`; `write_buffer_empty` (port-only) → drop. `login_failure` (acceptor-only) fires on the `Acceptor_handler` unconditionally — a reject is a pre-login-only event. Client-side always has `handler_` (the `Connection_handler`) set, so no branching.
...
- `read_success` has two `!state_.is_closing()` guards. The first precedes `process_packet`, so a packet already in flight when the close decision lands is discarded unprocessed. The second precedes the re-arm of `socket_.async_read()`, so no reads are queued after the close decision.
...
- Packet field validation (`Packet_error::invalid_field_value`) belongs in `Tcp_connection`'s `process_*` functions, after `read(...)` and before any side effect — e.g. `response.next_sequence_number == 0` in `process_login_accepted`. Watch `process_sequenced_data`'s `has_session_ended_` check: the flag lives on `Connection`, but it yields a `Packet_error` like any other guard, so it belongs ahead of the side effects — here the heartbeat count bump.
